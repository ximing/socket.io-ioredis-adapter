/**
 * Created by ximing on 17/5/4.
 */
const uid2 = require('uid2');
const IoRedis = require('ioredis');
const msgpack = require('notepack.io');
const Adapter = require('socket.io-adapter');
const debug = require('debug')('socket.io-ioredis-adapter');
...
function adapter(uri, opts) {
    ...
    const requestsTimeout = opts.requestsTimeout || 1000;
    const withChannelMultiplexing = false !== opts.withChannelMultiplexing;

    // init clients if needed
    function createClient() {
        if (uri) {
            // handle uri string
            return new IoRedis(uri,opts) //redis(uri, opts);
        } else {
            return new IoRedis(opts) //redis(opts);
        }
    }
    ...
    function Redis(nsp) {
        ...
        sub.subscribe([this.channel, this.requestChannel, this.responseChannel], (err) => {
